...
- Scratch / temp data: ALWAYS use the workspace project root's own `tmp/` directory — set `TMPDIR="$PWD/tmp"` when running from the project root (where opencode was launched) — for sandboxes, eval runs, scratch files, and any throwaway output. NEVER use `/var/folders/...`, `/tmp`, or `os.tmpdir()` defaults — those paths are outside the workspace permission scope and will fail to read back. Example: `TMPDIR="$PWD/tmp" bun ./.opencode/evals/lib/opencode-behavioral-runner.ts`.
