...
1. Reads task list from the active unified `plan.md` (auto-loaded by plan-autoload plugin)
2. Reads `.opencode/state/active-plan.json` to discover all write targets
3. For multi-project plans, a single unified plan.md covers all targets; state is centralized at `$WORKSPACE_ROOT/docs/specs/{feature-slug}/state/`
4. Reads feature-local state from `$WORKSPACE_ROOT/docs/specs/{feature-slug}/state/`, especially `implementer-work.md` and prior task execution files
...
9. Each task reads dependency execution/validator state before coding and writes state to `$WORKSPACE_ROOT/docs/specs/{feature-slug}/state/`
