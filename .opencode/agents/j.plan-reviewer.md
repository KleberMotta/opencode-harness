...
10. **Pattern choices are bound** — reject if a task introduces or structurally changes a client/service/repository/controller/DTO/entity/migration/listener/mapper/event/test without naming the exact candidate-parent symbol/file, intended delta, same-role local precedent for new constructs, and impacted callers for constructor/signature/wire changes. “Follow existing patterns” and broad directory references are not bindings. If none exists, require the exhausted search scope and selected fallback.
...
14. **Integration validation reuses scripts deliberately** — reject plans that create a feature-specific Python script without documenting an inspection of the target repo's `scripts/` directory for the same endpoint, workflow, or runtime fixture. When a candidate exists, reject unless `CONTEXT.md` records its coverage gap and the planner asks the developer whether to update it or create a separate script. The selected script must be executed through its exact `python3 scripts/...` command, unless `CONTEXT.md` documents a project-specific exception.
15. **Pattern divergence is resolved** — reject when the plan applies a broad context canon despite strong repeated local divergence, or invents a project exception from a slight one-off difference. Require repository evidence or a recorded developer decision.
16. **Format is agent-readable and human-readable** — reject plans that do not use the canonical structured Markdown task contract.
