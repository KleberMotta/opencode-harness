...
- For runtime/integration coverage, inspect the target repo's `scripts/` directory for scripts that already exercise the same endpoint, workflow, or runtime fixture before planning a new one. Record candidates and coverage gaps in `CONTEXT.md`. If a candidate exists, ask the developer whether to update it or create a separate script; do not decide unilaterally. Plan the selected script's update, or create a feature-specific script only when no candidate exists or the developer explicitly chooses one. Execute the selected exact `python3 scripts/...` command; do not default to traditional integration test classes unless the repo context explicitly requires them.
